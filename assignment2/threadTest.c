...
/* function names are obscenely lengthy */
#define up(x)     binary_semaphore_up(x)
#define down(x)   binary_semaphore_down(x)
...
/* Creates a semaphore with initial
   value `init' and returns it */
semaphore *
semaphore_create(int init)
{
  semaphore *s;
  
  s = &sems[sid++];
  s->s1 = binary_semaphore_create(1);
  s->s2 = binary_semaphore_create(init > 0 ? 1 : 0);
  s->val = init;
  return s;
}
